...
Using summer mean standard deviation in the SMB calculation ($\rm SMB_{JJA}$) gave smaller errors than other simplifying scenarios (Figure~\ref{fig:smb}), confirming recent results by \citet{rau-rogozhina-2013}. However in all four cases, large SMB errors occurred along the margin where most of the melt processes take place.

% ----------------------------------------------------------------------

\section{Conclusion}

Using gridded climate products, temperature variability can be assessed quantitatively at the continental scale. The standard deviation of daily mean temperatures is highly variable both spatially and seasonally, and this variability significantly affects the accuracy of annual PDD and SMB as estimated by the \citet{calov-greve-2005} or the equivalent \citet{reeh-1991} formulation. Numerical glacier models that use an annual PDD scheme should implement spatially and seasonally variable standard deviations of mean daily surface temperature in order to more realistically capture patterns of surface melt.
